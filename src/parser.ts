...
const regex =
	/(?<node>(?<ignoreBlock>(?:<!-- prettier-ignore-start -->|\[%# prettier-ignore-start #%\])[\s\S]*(?:<!-- prettier-ignore-end -->|\[%# prettier-ignore-end #%\]))|(?<comment>\[%#[\S\s]*?#%\])|\[%(?<startDelimiter>[-+]?)\s*(?<statement>(?<keyword>(?!GET|CALL|SET|DEFAULT|INSERT|INCLUDE|PROCESS|USE|RETURN|STOP|CLEAR|META|TAG|DEBUG)[A-Z]+|.*BLOCK)('([^']|\\')*'|"([^"]|\\")*"|[\S\s])*?)\s*(?<endDelimiter>[-+]?)%\]|\[%(?<startDelimiterEx>[-+]?)\s*(?<expression>'([^']|\\')*'|"([^"]|\\")*"|[\S\s]*?)\s*(?<endDelimiterEx>[-+]?)%\]|(?<scriptBlock><(script)((?!<)[\s\S])*?>((?!<\/script)[\s\S])*?\[%[\s\S]*?<\/(script)>)|(?<styleBlock><(style)((?!<)[\s\S])*?>((?!<\/style)[\s\S])*?\[%[\s\S]*?<\/(style)>))/;
